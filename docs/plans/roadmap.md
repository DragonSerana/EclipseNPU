...
## RTL 支线（v0.6 新增，2026-09 重排，详见 docs/plans/rtl-plan.md）

- 定位：学习支线（流水线/握手/反压/存储系统的设计侧理解）+ cycle 模型校准手段，**不是流片**；只写核心单元，外围用行为级模型。
- 关系反转：cmodel 从"硬件本身"升级为合同/golden，RTL 是实现；Verilator 挂进同一 harness，同一指令流双跑，在指令边界比对 SRAM/DDR + cycle（NVDLA cmod/RTL 双轨实践）。
- 节奏：R1 valid/ready 流水与反压 → R2 bank 化 SRAM 与仲裁 → R3 DMA 引擎 + descriptor 取指 → R4 16×16 MAC 阵列 + 顶层跑通 H1 指令流 → R5 LUT/EWISE/REDUCE 按需 → R6 bit-exact 加法器可选。每层独立 done，独立可停。
- 排序依据是"惩罚发生在哪"：存储系统（bank/端口/仲裁）才是惩罚的发生地，算术阵列排在它后面；done 标准是"cycle 能手算 + 波形与手算一致"，不是"与 cmodel 逐 bit 一致"。
...
| H5 | 硬件的物理极限 | 双后端对比报告 |
| RTL 支线 | 从顶到底的硬件理解（差异化优势） | cmodel↔RTL 逐指令对照 + 校准后的时序参数表（bit-exact 加法器为可选单元 R6） |
